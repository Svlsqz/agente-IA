    ...
    def respond(chatbot: List, message: str, chat_type: str, app_functionality: str) -> Tuple:
        ...
        if app_functionality == "Chat":
            ...
            if chat_type == "Q&A with stored SQL-DB":
                ...
                if os.path.exists(APPCFG.sqldb_directory):
                    ...
                    def validate_result(state):
                        if not state["result"]:  # Si la consulta SQL no devolvió datos
                            return {"output": "No hay información disponible."}
                        return state  # Si hay datos, continuar con la ejecución normal

                    # Construcción del chain
                    chain = (
                            RunnablePassthrough.assign(query=write_query)  # Generar la consulta SQL
                            .assign(result=itemgetter("query") | execute_query)  # Ejecutar la consulta
                            .assign(output=RunnableLambda(validate_result))  # Validar si hay resultados
                            | answer  # Si hay datos, pasar al LLM
                    )
            ...
            # If we want to use langchain agents for Q&A with our SQL DBs that were created from CSV/XLSX files.
                ...
                response = APPCFG.openai_client.embeddings.create(
                    input=message,
                ...
                llm_response = APPCFG.openai_client.chat.completions.create(
